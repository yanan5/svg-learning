(function () {
  ...
  const {select: select$1, scaleLinear, max: max$3, scaleBand, axisLeft: axisLeft$1, csv: csv$2, axisBottom: axisBottom$1} = d3;

  const svg$1 = select$1('svg');

  const width = +svg$1.attr('width');
  const height = +svg$1.attr('height');
  ...
  const render = data => {
    ...
    const yAxis = axisLeft$1(yScale);
    const xAxis = axisBottom$1(xScale);
  ...
  csv$2('./data/population.csv').then(data => {
